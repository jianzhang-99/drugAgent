<template>
  ...
  >
    ...
    <div v-if="!report" class="flex-1 flex flex-col items-center justify-center p-8 text-center">
      <div class="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center mb-4">
        <FileText size="32" class="text-slate-400" />
      </div>
      <h3 class="text-sm font-bold text-slate-700 mb-2">暂无报告</h3>
      <p class="text-xs text-slate-400 mb-4">点击结果卡片查看详细分析报告</p>
      <button
        @click="$emit('close')"
