...
<div className="p-4 flex items-center justify-between">
  ...
  {isSidebarOpen && <span className="font-bold text-lg tracking-tight text-slate-800 truncate">横渡智能体</span>}
</div>
{isSidebarOpen && (
<button
...
<header className="h-14 border-b border-slate-100 px-6 flex items-center justify-between bg-white/80 backdrop-blur-md sticky top-0 z-10">
  <div className="flex items-center gap-2">
            <span className="text-slate-400 text-sm font-medium">
              {activeView === 'TASKS' ? '任务调度看板' : activeView === 'KNOWLEDGE' ? '知识大脑' : activeView === 'SETTINGS' ? '系统配置' : '智能体审查工作台'}
            </span>
...
<div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 pb-32">
  ...
  <div className="flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in slide-in-from-bottom-4">
    ...
    <p className="text-slate-500 mb-10 text-sm">直接描述您的监管需求，智能体将自动分发到对应的工作流</p>
...
<div className="bg-slate-100/50 px-5 py-3.5 rounded-2xl rounded-tl-sm text-slate-400 text-[14px] flex items-center gap-2">
  <Loader2 size={16} className="animate-spin" /> 智能体正在执行深层编排工作流...
</div>
</div>
)}
...
<div className="w-full max-w-4xl mx-auto bg-white rounded-3xl shadow-[0_-4px_24px_-8px_rgba(0,0,0,0.08)] border border-slate-200 p-1.5 transition-all focus-within:ring-4 focus-within:ring-blue-100 focus-within:border-blue-400">
  ...
  rows="2"
  placeholder={activeSession ? "向智能体追加要求或提供更多材料..." : "描述您的监管需求，例如：检测这两份标书文件是否雷同..."}
  ...
  <p>管理智能体的长期记忆与审查准则，包括 RAG 向量切片库和规则引擎字典。</p>
  </div>
  );
  };
  ...
  { id: 'ENGINE', label: '智能体引擎偏好', icon: Cpu },
  ...
  <p className="text-sm text-slate-500">管理当前账户的基础信息、智能体底层大模型参数以及文档处理的合规留痕策略。</p>
  </div>
  ...
  <p className="text-xs text-slate-500 mt-1">调整支撑横渡智能体运行的核心大语言模型及推理参数。</p>
  </div>
  ...
  <p className="text-xs text-slate-500 mt-1">控制上传至智能体的敏感文件如何被存储与销毁。</p>
  </div>
  ...
  <div>
  <h4 className="font-bold text-slate-800 text-sm mb-1 flex items-center gap-2">
  <Terminal size={16} className="text-slate-400" /> 默认展开思考图谱 (Trace)
  </h4>
  <p className="text-xs text-slate-500 max-w-sm leading-relaxed">
  工作台中生成审查报告时，默认展示底层的调用节点及执行日志，适合审计员及高级审核专家。
  </p>
  </div>
