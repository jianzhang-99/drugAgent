...
AI 给出⁠的示例输出如下，接‌下来我们化身为 “​老板”，只需要对 ‎AI 给出的需求文‌档进行评审即可：
    ...
    2. **用户身份及需求**
        - **患者及家属（C端用户）**：担心漏服、错服药物，缺乏对药物相互作用的认知。他们需要服药提醒、配伍禁忌查询、说明书通俗化解读。
        - **慢性疾病群体（高频用药用户）**：需要长期服药，高度关注药物耐受性、肝肾功能影响、定期复诊指标以及日常饮食与药物冲突。
        - **从业人员（B端用户）**：要求快速检索最新药典法规、处方合规性审查、医保政策解读以及行业监管动态，以降低医疗差错。
    
    ### 二、功能板块规划
    #### （一）问题识别与分类
    1. **自然语言处理**：用户以文字或语音倾诉用药监管困扰，AI凭借自然语言处理技术，精准提炼问题核心，判别用户情绪状态，如愤怒、迷茫、伤心等。
    2. **意图智能归类**：将收集到的问题，自动归入用药咨询、配伍禁忌、不良反应鉴别、政策法规查询、合规审查指导等预设类别，为后续的专业解答做好准备。
    
    #### （二）个性化指导服务
    1. **智能药理问答**：针对用户疑问，AI 依托国家药监局数据库、权威药典及临床指南，生成专业的解答。例如，当用户询问“布洛芬和对乙酰氨基酚能否交替吃”时，AI会结合用户年龄和症状，给出明确的用药限制及风险提示。
    2. **健康与科普推送**：为大众推送“抗生素误区”、“高血压安全用药指南”等图文/视频课程，助力用户系统学习安全用药知识。
    3. **风险预警播报**：实时共享国内外的药品召回事件、药监局飞检通报及修改说明书公告，为用户提供及时的行业安全风向标。
    
    #### （三）特色辅助功能
    1. **处方合规审查**：面向医疗机构或连锁药店，AI基于规则引擎和知识库，自动审查电子处方的超说明书用药、配伍禁忌等问题。
    2. **数字用药管家**：依据医嘱及说明书，AI为用户自动生成专属用药日历，涵盖精准的服药时间、剂量计算、餐前/餐后禁忌提醒等细节。
...
AI 提供的优化后系统提示词：
    ...
    扮演深耕医疗健康与用药监管领域的专家。开场向用户表明身份，告知用户可咨询用药和合规难题。围绕普通患者、慢性病患者、医药从业者三种身份提问：普通患者询问日常用药疑惑或轻微不良反应；慢性病患者询问长期服药的耐药性及复诊指标；医药从业者询问处方合规审查与最新药典法规。引导用户详述症状或核心诉求，以便给出专业严谨的解答。
...
1）首先初始化 ChatC⁠lient 对象。使用 Spring 的构造器注入方‌式来注入阿里大模型 dashscopeChatMod​el 对象，并使用该对象来初始化 ChatCli‎ent。初始化时指定默认的系统 Prompt 和基于内存‌的对话记忆 Advisor。代码如下：
    ...
    public class LoveApp {
        ...
        private static final String SYSTEM_PROMPT = "扮演深耕医疗健康与用药监管领域的专家。开场向用户表明身份，告知用户可咨询用药和合规难题。" +
                "围绕普通患者、慢性病患者、医药从业者三种身份提问：普通患者询问日常用药疑惑或轻微不良反应；" +
                "慢性病患者询问长期服药的耐药性及复诊指标；医药从业者询问处方合规审查与最新药典法规。" +
                "引导用户详述症状或核心诉求，以便给出专业严谨的解答。";
...
3）编写单元测试，测试多轮对话：
    ...
    class LoveAppTest {
        ...
        void testChat() {
            ...
            message = "我最近检查出高血压，需要长期服药，应该注意什么？";
            answer = loveApp.doChat(message, chatId);
            Assertions.assertNotNull(answer);
            
            message = "我刚才说我的什么病来着？刚跟你说过，帮我回忆一下";
            answer = loveApp.doChat(message, chatId);
            Assertions.assertNotNull(answer);
        }
    }
...
4）编写单元测试代码：
    ...
    void doChatWithReport() {
        ...
        String message = "你好，我是药剂师梁哥，我想让患者（慢性病群体）更配合用药，但我不知道该怎么做";
