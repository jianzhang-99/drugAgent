...
用户在过程⁠中遇到各种问题，如服药时间、‌配伍禁忌、处理不良反应等，都可以​向大师 AI 应用提问。应用‎能提供准确、有针对性的解答和‌建议，帮助用户解决实际问题。

场景示例：用户⁠询问 "布洛芬和对乙酰氨基酚能否交替吃？"，系统​能根据用户的具体情况（年龄、‎病史、当前用药等）提供个性化‌建议，并推荐相关的用药指导课程。
...
场景示例：⁠当用户在社区发布 "如‌何缓解高血压药物副作用" 的话题​时，系统能自动汇总相‎关经验分享，同时提供‌专业建议和成功案例分析。

#### ⁠4. 处方审查‌         ​         ‎         ‌     

基于患者的身体指标、用药史‌和过敏史，帮助审​查处方合理性，‎提供用药建议。

场景示例：⁠系统分析患者的处方信息‌和合规需求，提​示潜在的配伍禁忌‎，并提供替换药物建议。
...
首先 AI 原本就拥有一些通用的⁠知识，对于不会的知识，还可以利用互联网搜索。但是这些都是从网络获‌取的、公开的知识。对于企业来说，数据是命脉，也是自己独特的价值，​随着业务的发展，企业肯定会积累一波自己的知识库，比如往期患者的用‌药咨询和成功案例、以及自家的课程，如果让 AI 能够利用这些‌知识库进行问答，效果可能会更好，而且更加个性化。
...
💡 大家⁠在学习 RAG 的‌过程中，可以利用 ​AI 来生成文档，‎提供一段示例 Pr‌ompt：
    ...
    帮我生成 3 篇 Markdown 文章，主题是【常见问题和回答】，3 篇文章的问题分别针对高血压、、糖尿病的状态，内容形式为 1 问 1 答，每个问题标题使用 4 级标题，每篇内容需要有至少 5 个问题，要求每个问题中推荐一个相关的课程，课程链接都是 https://www.codefather.cn
...
编写单元测试代码，故意提问一个文档内有回答的问题：
    ...
    void doChatWithRag() {
        ...
        String message = "我最近被诊断出高血压，需要长期服药，有哪些注意事项？";
...
1）利用 ⁠RAG 知识库，实‌现 “通过患者的情况推荐可用药物”‎ 功能。

参考思路：⁠新建一个药品文‌档，每行数据包含一​种药物的基本信息（‎比如适应症、禁忌‌、不良反应）。
