 ...
 * 服务承诺语义分析器（W-P3）。
 *
 * <p>职责：对两份标书的服务承诺章节进行 LLM 语义判断，识别服务等级和时效组合高度同源、
 * 承诺逻辑同源、表达不同但服务体系配置基本一致等软性抄袭特征。
 *
 * <p>不命中典型情况：
 * <ul>
 *   <li>仅行业标准服务水平协议（如"7x24小时客服"通用表述）</li>
 *   <li>仅通用服务承诺表述（如"提供培训服务"、"提供技术支持"等无差异化描述）</li>
 *   <li>服务等级和时效存在实质差异，具有独立编写痕迹</li>
 * </ul>
 *
...
public class ServiceCommitmentSemanticAnalyzer implements TenderSemanticAnalyzer {

    private static final String ANALYZER_CODE = "W-P3";
    private static final String COMPARE_TOPIC = "服务承诺";
    ...
    public List<RuleHit> analyze(TenderReviewData data) {
        if (data == null || data.getCompareScopes() == null || data.getCompareScopes().isEmpty()) {
            log.info("[ServiceCommitmentSemanticAnalyzer] 无可比对范围，返回空命中列表");
            return List.of();
        }
            ...
            for (int i = 0; i < docIds.size(); i++) {
                for (int j = i + 1; j < docIds.size(); j++) {
                    ...
                    if (candidates.isEmpty()) {
                        log.debug("[ServiceCommitmentSemanticAnalyzer] caseId={}, leftDoc={}, rightDoc={} 无候选片段",
                                caseId, leftDocId, rightDocId);
                        continue;
                    }

                    log.info("[ServiceCommitmentSemanticAnalyzer] caseId={}, leftDoc={}, rightDoc={}, 候选数={}",
                    ...
                    for (TenderSemanticJudgeReq candidate : candidates) {
                        ...
                        allHits.add(hit);
                        log.info("[ServiceCommitmentSemanticAnalyzer] caseId={}, 命中规则 W-P3, confidence={}, conclusion={}",
    ...
    private RuleHit convertToRuleHit(TenderSemanticJudgeResp resp, CompareScope scope, TenderSemanticJudgeReq req) {
        ...
        hit.setWeight(resp.getSuggestedWeight() != null ? resp.getSuggestedWeight() : 70);
        hit.setConfidence(resp.getConfidence());
        hit.setMatchedValue("semantic:" + resp.getConclusion());

        String triggerSummary = String.format("两份标书服务承诺章节存在语义同源特征（置信度 %.2f）：%s",
        ...
        log.info("[ServiceCommitmentSemanticAnalyzer] LLM_SEMANTIC_RULE hit created - ruleCode=W-P3, confidence={}, reason={}",
